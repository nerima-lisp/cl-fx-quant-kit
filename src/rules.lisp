...
CL-PROLOG's macro-first DSL, while all domain data remains an ordinary value
record that can be created and tested independently."
  ...
  (multiple-value-bind (solution foundp)
    ...
    (make-risk-decision-result
     (cl-prolog:solution-binding '?action solution)
     (cl-prolog:solution-binding '?reason solution)
     state)))
